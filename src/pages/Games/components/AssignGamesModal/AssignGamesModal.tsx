...
import {useAssignGameMutation} from '@app/api/common/gameApiSlice';
...
import styles from './AssignGamesModal.module.scss';

type AssignGameModal = {
	showModal: boolean;
	selectedGame: number | null;
	setShowModal: (value: 'assign' | 'create' | null) => void;
};

export const AssignGameModal: FC<AssignGameModal> = ({selectedGame, showModal, setShowModal}) => {
	...
	const [assignGame, {isLoading: isAssigning}] = useAssignGameMutation();
		...
		const onObservedsRequest = async () => {
			try {
				...
				const selected = observeds.results
					.filter((user) => {
						if (!user.games || !selectedGame) {
							return false;
						}

						return user.games.includes(selectedGame);
					})
					.map((item) => item.id);
	...
	return selectedGame === null ? null : (
		<Modal active={showModal} handleClose={handleCloseModal}>
			<ModalLayout
				title={'Назначить/снять игру с наблюдаемых'}
				content={
